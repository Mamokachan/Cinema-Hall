...
function showBooked(places) {
    ...
    const reserved = places.reserved;
...
function seatClick(dbIndex) {
    const getData = `${
    'movieImg=' +
    DATABASE[dbIndex].img +
    ';' +
    'movieLink=' +
    DATABASE[dbIndex].link +
    ';' +
    'movieName=' +
    encodeURI(DATABASE[dbIndex].movie)
  }`;
    ...
    seatsArr.forEach(eachSeat =>
        eachSeat.addEventListener('click', function clickID() {
          ...
          seat + 1
        } место.</p> <p>Приятного просмотра!</p> <a href=${
          URL + '?' + allInf
        } target='_blank'class="shine-button">Перейдите на страницу просмотра</a>`;
            }
        })
    );
}
...
function renderSeans() {
    ...
    seansRoot.forEach((current, index) => {
        ...
        for (let i = 0; i < session.length; i++) {
            links += `<span class="el__sesion__link" id=${
        index + '-' + i
      } onclick="sessionClick(this)"> Сеанс ${session[i].date}  </span> <br>`;
        }
